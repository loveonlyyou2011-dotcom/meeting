...
- 회의 중 실시간 자막은 **Gemini Live API**(WebSocket 스트리밍)로 처리해 지연을 수 초
  이내로 줄였습니다. 화자 라벨은 붙지 않으며(단순 참고용), 종료 시점에 diarization과
  전체 전사를 시간축으로 정렬해 화자별 발화록을 별도로 만듭니다.
...
- Gemini 오디오 처리 비용: 실시간 자막(Live API 세션) + 종료 시 전체 전사(REST) 두
  경로로 처리됩니다. Live API는 무료 티어에서 요청 횟수 한도가 아니라 **동시 세션 3개**
  제한만 적용되므로, 개인이 순차적으로 회의를 진행하는 용도라면 하루 사용량 걱정은
  거의 없습니다.

## 실시간 자막(Live API) 관련 참고사항

- **세션 길이 제한**: 압축 없이는 오디오 세션이 15분으로 제한됩니다. 백엔드가 9분마다
  `session_resumption` 핸들로 자동 재접속하며, 재접속 순간 자막이 1~2초 짧게 끊길 수
  있습니다(정상 동작입니다).
- **연결 실패 시 폴백**: Live API 연결이 실패해도 녹음(15초 청크 업로드)과 회의 종료 후
  화자분리·회의록 생성 경로는 전혀 영향받지 않습니다. 화면 상단 배지가 "연결 끊김"으로
  표시되면 그 구간은 실시간 자막만 안 보일 뿐, 최종 회의록에는 정상적으로 포함됩니다.
- **모델명**: `backend/live_transcribe.py`의 `LIVE_MODEL`이 preview 모델이라 이름이
  바뀌거나 만료될 수 있습니다. 연결이 안 되면 [AI Studio](https://aistudio.google.com/)에서
  사용 가능한 `-live-` 계열 모델명을 확인해 값을 교체하세요.
- **브라우저 요구사항**: AudioWorklet은 `localhost` 또는 HTTPS에서만 동작합니다(보안 제약).
  폰에서 접속하려면 이 조건도 함께 만족해야 합니다.
